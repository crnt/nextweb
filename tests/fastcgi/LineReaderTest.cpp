...
LineReaderTest::testReadLinesWith() {
	...
	typedef typename Sequence::const_iterator IteratorType;
	
	utils::Range<IteratorType> result;
	fastcgi::LineReader<IteratorType> reader(value.begin(), value.end());

	std::size_t i = 0;
	for (; reader.readLine(result); ++i) {
		CPPUNIT_ASSERT_EQUAL(std::string(values[i]), makeString(result));
	}
